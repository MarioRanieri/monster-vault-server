...
abstract class E2EBaseTest {
    ...
    void setUpE2E() throws Exception {
        ...
        // Mock AuthService: qualsiasi credenziale → token pair valido
        String accessToken = jwtUtil.generateAccess("testadmin");
        String refreshToken = jwtUtil.generateRefresh("testadmin");
        var authResponse = new com.monstervault.service.AuthResponse(accessToken, refreshToken);
        when(authService.authenticate(any(), any())).thenReturn(Optional.of(authResponse));
        when(authService.refresh(any())).thenReturn(Optional.of(authResponse));
