...
- **Default order leads with photographed cans** — the "recently photographed" sort keyed only on `photoAt`, but many photographed cans have `photoAt=0` (legacy imports) and mixed in with the 1510 photo-less cans, so a guest opened onto a wall of placeholders. Photo presence is now the primary sort key; nothing is filtered out.
- **Text contrast toward WCAG AA** — `--text3` for small secondary text was raised (`#555`→`#7a7a7a` dark, `#94a3b8`→`#64748b` light) to clear the AA contrast floor.
- **No console 401 for guests** — the mount-time `POST /api/auth/refresh` (which guests always 401'd) is now gated behind a non-HttpOnly `mv_auth` session hint set on login, so guests skip the pointless request; added the standard `mobile-web-app-capable` meta.
- **Stray "NO" promo badge** — cans with a legacy literal `promo="NO"` no longer render a promo badge on card/list/detail and are excluded from the Promo filter and count, via a shared `hasPromo()` predicate.
...
### Changed
- **Incremental grid rendering** — the grid/list/wall now mount the first 60 cans and grow by 60 as a bottom sentinel scrolls into view (IntersectionObserver, 600px preload), instead of putting all ~1866 nodes in the DOM up front; the "X of Y cans" count still reflects the full filtered set.
- **Filter state consolidated** — the 14 separate filter `useState`s in `App.tsx` (duplicated across declaration, restore, filter, reset, share/apply and stats-click) collapsed into a single `filters` object with a `setFilter` helper and a `NO_FILTERS` reset constant; no behaviour change.
- **"Full" accent tokenized** — the hardcoded `#00b4ff` repeated across the Full chip, badge and hero stat became a `--full` / `--full-rgb` CSS token (brighter cyan, with a darker light-theme variant), dropping the stray inline colour in `Hero`.
- **eBay monitor moved to the cloud** — the standalone `ebay-monitor/` radar no longer needs a PC left on: it runs on **GitHub Actions** (cron every 2h, one pass per run) with state on **MongoDB Atlas** (`ebay_seen`/`ebay_blacklist`, dedicated collections that never touch `cans`), entirely free; secrets come from GitHub Secrets. Removed the polling loop, anti-standby hack and SQLite; deleted the VLM photo-matching skeleton and the failed CLIP/DINOv2/OCR experiments (image recognition can't tell the can variants apart, so blacklist curation is the strategy). The listing-age window widened 2.5h→3.5h to absorb cron delays. Telegram commands are drained once per run: `/delete`, plus new **`/add <word>`** (curate the blacklist live — persisted to Mongo, with a guard against blinding the radar) and **`/list`**. The base blacklist was extracted to a versioned `blacklist.txt`. The workflow runs every **5 min** to drain commands quickly (the repo is public, so Actions minutes are free), while the eBay sweep stays gated to ~2h via a Mongo timestamp; a **pinned banner** in the chat explains the command latency and is preserved by `/delete`.
