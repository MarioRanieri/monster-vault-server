...
| Sonar `css:S7924` (contrasto) su badge, chip e messaggi | **12 restano segnalati: falsi positivi** | Sonar ignora l'alpha dello sfondo: vede `#00c850` su `rgba(0,200,80,.15)` come "stesso colore". Il contrasto vero (WCAG AA 4,5:1, tema scuro e chiaro) è calcolato e asserito da `frontend/src/styles/contrast.test.ts`; le righe sono marcate `NOSONAR`. Un contrasto nuovo va aggiunto a quel test, non "corretto" a occhio. |
...
| # | Voce | Asse | Impatto guest | Sforzo | Prio | Stato |
|---|------|------|---------------|--------|------|-------|
| 1 | I font progettati **non caricano in prod** (CSP blocca Google Fonts) | Visivo+Codice | Altissimo | M | **P0** | ✅ fatto |
...
| 6 | Stato filtri sparso in App.tsx (~30 useState, +14 filtri ×5 punti) | Codice | Nullo per il guest, alto per te | M | **P2** | ✅ fatto |
...
| 9 | Griglia renderizza tutte le ~1866 card in DOM | Codice (perf) | Basso-medio | M–L | **P2** | ✅ fatto (render incrementale, DOM 60 vs 1866) |
